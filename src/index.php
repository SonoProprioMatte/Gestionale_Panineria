    ...
    if ($action === 'login') {
        ...
        if ($name && $email && strlen($password) >= 8) {
            ...
            if ($check->fetch()) {
                ...
                // Rate limit: max 3 attempts per email in 10 minutes
                ...
                if ((int)$rateStmt->fetchColumn() >= 3) {
                    $error = 'Troppe richieste per questa email. Attendi qualche minuto.';
                } else {
                    // Remove old pending verifications for this email
                    ...
                    if ($result === true) {
                        // Store minimal info in session to identify the pending verification
                        ...
                        $error = 'Errore invio email: ' . $result . ' — controlla le credenziali SMTP nel docker-compose.yml';
                    }
                }
            }
        } else {
